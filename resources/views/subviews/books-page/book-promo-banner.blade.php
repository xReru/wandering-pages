...
<section class="bg-[#F9F8FF] py-12 px-4 sm:px-6 lg:px-8 font-['EBGaramond']">
    <div class="container mx-auto">
        <div class="flex flex-col md:flex-row items-center justify-between rounded-lg overflow-hidden shadow-lg"
             style="background: linear-gradient(98deg, #E1D9FF 43%, #B3A9D7 98%);">
            <!-- Left Section with Text and CTA -->
            <div class="p-6 md:p-8 lg:p-12 md:w-1/2 lg:w-2/5 flex flex-col justify-center h-full">
                <h2 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#1B1146] mb-4 font-primary tracking-[0.05em]">{{ $title }}</h2>
                <div class="flex items-center mb-6 font-['EBGaramond']">
                    <p class="text-[#838383] line-through mr-3 text-lg font-normal ">$ {{ number_format($originalPrice, 2) }}</p>
                    <p class="text-[#1B1146] font-bold text-xl sm:text-1xl font-normal font-semibold">$ {{ number_format($salePrice, 2) }}</p>
                </div>
            ...
            <div class="p-4 md:w-1/2 lg:w-3/5 flex items-center justify-center min-h-[22rem] w-full">
                <div class="relative h-72 sm:h-80 md:h-120 w-full flex items-center justify-center" x-data="bookPromoFan({{ json_encode($books) }})" x-init="init()">
                    <template x-for="(book, index) in books" :key="index">
                        <div class="absolute transition-all duration-700 book-cover cursor-pointer" :style="getBookStyle(index)" @click="setActiveBook(index)">
                            <img :src="book.image.startsWith('http') ? book.image : '/'+book.image" :alt="book.title" class="h-64 sm:h-72 md:h-80 lg:h-96 w-40 sm:w-48 md:w-56 lg:w-60 rounded-lg shadow-xl object-cover" style="font-family: 'EBGaramond', serif;">
                        </div>
                    </template>
