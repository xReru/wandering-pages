                            ...
                            @foreach($order->items as $item)
                                <div class="flex gap-4 items-start">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="mt-1 text-sm sm:text-base text-purple-600 font-medium">${{ number_format($item->price_at_time_of_order ?? $item->book->price, 2) }}</div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-4">
                                <div class="space-y-2 text-sm">
                                    <div class="flex justify-between text-gray-600">
                                        <span>Subtotal</span>
                                        <span class="text-purple-600 font-medium">${{ number_format(($order->total_amount ?? 0) - ($order->shipping_fee ?? 0), 2) }}</span>
                                    </div>
                                    <div class="flex justify-between text-gray-600">
                                        <span>Shipping Fee</span>
                                        <span class="text-purple-600 font-medium">${{ number_format($order->shipping_fee ?? 0, 2) }}</span>
                                    </div>
                                    <div class="border-t border-gray-200 pt-2 mt-2">
                                        <div class="flex justify-between">
                                            <span class="font-medium text-gray-800">Total</span>
                                            <span class="font-bold text-purple-700">${{ number_format($order->total_amount ?? 0, 2) }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <a href="{{--route('books.index')--}}" 
                                       class="w-full inline-flex justify-center items-center px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition-colors duration-200 shadow-sm">
                                        Buy Again
                                    </a>
                                </div>
                            </div>
