    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ tab: 'completed' }" x-cloak>
        ...
        <div class="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-6 border-b border-gray-200">
            <button @click="tab = 'completed'" 
                :class="tab === 'completed' ? 'bg-purple-100 border-b-2 border-purple-600 font-medium text-purple-900' : 'bg-transparent text-gray-600 hover:text-purple-700'" 
                class="px-4 py-2.5 focus:outline-none transition-all duration-200 rounded-t-lg text-sm sm:text-base">
                Completed Orders
            </button>
            <button @click="tab = 'cancelled'" 
                :class="tab === 'cancelled' ? 'bg-purple-100 border-b-2 border-purple-600 font-medium text-purple-900' : 'bg-transparent text-gray-600 hover:text-purple-700'" 
                class="px-4 py-2.5 focus:outline-none transition-all duration-200 rounded-t-lg text-sm sm:text-base">
                Cancelled Orders
            </button>
        </div>
